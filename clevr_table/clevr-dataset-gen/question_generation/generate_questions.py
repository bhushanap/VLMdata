...
# Inputs
parser.add_argument('--input_scene_file', default='../output/CLEVR_misc_scenes.json',
...
parser.add_argument('--template_dir', default='CLEVR_1.0_templates_iteration_5',
...
# Misc
parser.add_argument('--reset_counts_every', default=2100, type=int,
...
def add_empty_filter_options(attribute_map, metadata, num_to_add):
  ...
  if metadata['dataset'] == 'CLEVR-v1.0':
    attr_keys = ['Size','Material', 'Shape', 'Color'] #'
  else:
    assert False, 'Unrecognized dataset'
...
def instantiate_templates_dfs(scene_struct, template, metadata, answer_counts,
  ...
  for state in final_states:
    ...
    # print(state['vals'].items())
    error = False
    for name, val in state['vals'].items():
      # print("name",name)
      # print("val",val)
      # print("text",text)
      if val=='center' or val=='top' or val=='bottom' or val=='left' or val=='right' or val=='top left' or val=='top right' or val=='bottom left' or val=='bottom right':
        if not "table" in text:
          error = True
          print("no relation for table error")
          break
      if "table" in text and val=="table":
        error = True
        print("double table error")
        break
      if val=='farthest' or val=='nearest':
        if "Are" in text or "many" in text or "number" in text:
          print("nearest farthest error")
          error = True
          break
      if val=="inside":
        if "block" in text or "cube" in text:
          print("inside cube error")
          error = True
          break
      if "inside" in text and val=='bowl':
        print("bowl inside error")
        error = True
        break
      if val=="below" or val=="above":
        if "bowl" in text or "bowl" in text:
          print("below bowl error")
          error = True
          break
    
      # if name==""
      # print("answers",answers)
      # print("sq",structured_questions)
      if val in synonyms:
